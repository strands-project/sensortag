...
class SensorTag:
    ...
    def notification_loop_len( self, handle, cbs ):
        while True:
...
class SensorCallbacks:
    ...
    def temperature(self,v):
        objT = (v[1]<<8)+v[0]
        ambT = (v[3]<<8)+v[2]
        ot, at = temperature(objT, ambT)
        return at
        
    def humidity(self, v):
        rawT = (v[1]<<8)+v[0]
        rawH = (v[3]<<8)+v[2]
        hm = humidity(rawT, rawH)
        return hm
...
def main():
    ...
    #while True:
     #try:   
      ...
      #rawcal_t = tag.char_read_hnd(0x25)
      #print rawcal_t
      while(True):
        tag.notification_loop_len(0x21,cbs)
     #except:
     # pass
